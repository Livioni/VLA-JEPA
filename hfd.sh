...
finalize_filelist() {
    ...
    : > "$mf"
    awk -F'\t' -v mf="$mf" '!seen[$2]++ { print >> mf; n++; s+=$1 } END { printf "%d %.0f\n", n, s }' \
        ...
        while :; do
            ...
            read -r now files < <(awk -F'\t' '
                ...
                { p=$2; sub(/^\.\//,"",p)
                  ...
                  if (p in want) { b+=$1; d++ } }
                # The default awk numeric-to-string conversion switches to scientific
                # notation for large byte counts (for example 2.15331e+09). Bash
                # arithmetic cannot consume that form, so force a plain integer.
                END { printf "%.0f %d\n", b * 512, d - a }
...
# hfd versions before this one let awk serialize large totals in scientific notation,
# which Bash arithmetic cannot read. Rebuild those cached totals exactly from the manifest.
if [[ ! "$TOTAL_SIZE" =~ ^[0-9]+$ && -f .hfd/manifest ]]; then
    read -r TOTAL_FILES TOTAL_SIZE < <(awk -F'\t' '{ n++; s+=$1 } END { printf "%d %.0f\n", n, s }' .hfd/manifest)
    printf '%s %s\n' "$TOTAL_FILES" "$TOTAL_SIZE" > .hfd/filelist_stats
    printf '%s %s\n' "$TOTAL_FILES" "$TOTAL_SIZE" > .hfd/repo_info
fi
